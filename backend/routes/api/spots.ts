...
function transformSpot(
  wholeSpot: any // temporarily use any to bypass strict typing
): object {
  ...
  return {
    ...
    avgRating: reviews.reduce((a: any, i: { stars: any; }) => a + i.stars, 0) / reviews.length,
  };
}
...
router.get('/:spotId', async (req: Request, res: Response) => {
  ...
  const ret = {
    ...
    avgStarRating: reviews.reduce((a: number, i: { stars: number }) => a + i.stars, 0) / reviews.length,
    SpotImages: images,
    Owner: owner,
  };
...
router.get('/:spotId/reviews', async (req, res) => {
  ...
  const out = reviews.map((r: { [x: string]: any; user: any; images: any; }) => {
